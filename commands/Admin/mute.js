...
exports.run = async (client, message, args) => {
    ...
    message.channel.send(`<@${tomute.id}> został zmutowany na ${ms(ms(mutetime))}`).then(msg => msg.delete(30000));

    setTimeout(function () {
        tomute.removeRole(muterole.id);
        message.channel.send(`<@${tomute.id}> został odmutowany`).then(msg => msg.delete(30000));
